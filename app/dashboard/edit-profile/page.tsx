...
export default function EditProfilePage() {
  ...
  const [profile, setProfile] = useState<{ role: 'player' | 'coach' } | null>(null)
      ...
      <div className="min-h-screen bg-slate-900 px-4 py-8">
        <div className="mx-auto max-w-4xl">
          <div className="rounded-lg bg-slate-800 p-8 shadow-xl border border-slate-700">
            ...
            <p className="text-slate-300 mb-4">{error || 'Profile not found'}</p>
            <Link
              href="/dashboard"
  ...
  return (
    <div className="min-h-screen bg-slate-900 px-4 py-8">
      <div className="mx-auto max-w-4xl">
        <div className="mb-6">
          <Link
            href={profile.role === 'player' ? '/athlete/dashboard' : '/coach/dashboard'}
        ...
        <div className="mb-8 rounded-lg bg-slate-800 p-8 shadow-xl border border-slate-700">
          <h1 className="mb-2 text-3xl font-bold text-white">
            {profile.role === 'player' ? 'Edit Player Profile' : 'Edit Coach Profile'}
          </h1>
        ...
        <div className="rounded-lg bg-slate-800 p-8 shadow-xl border border-slate-700">
          {profile.role === 'player' ? (
            <PlayerProfileForm isEditMode={true} />
          ) : (
